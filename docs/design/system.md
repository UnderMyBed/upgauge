...
at a glance, and interleaved they are not. Where the table carries a rank column, a below-floor
row takes no number: that cell reads `—`. Excluded from ranking, not from sight.

**A ranked table is one whose order the product chose** — the four entity pages and the
`/watch` presets. `/explore` renders the order its own query specifies, and re-ordering the rows
afterwards would break the promise that the page shows the query you wrote. `s=` carries a sort
key *and* a direction; where it is absent the query still resolves to one, since `render.ts`
falls back to the first selected measure. So the exemption does not rest on the visitor having
spelled a sort — it rests on the order being a property of the permalink either way.
`s=departures_performed` ascending is the case that makes it concrete: someone explicitly asking
to see the sparsest rows first, whom a partition would answer by moving them to the foot. The
floor treatment applies to every row there exactly as it does elsewhere; only the ordering
...
- **Every control is a real anchor, never a button or an input.** A native form GET cannot emit
  ...
  dropping `aircraft_type` at route grain would leave a reader wondering where it went.
- **Derived measures render as dashed chips**, read from the catalog's own `is_additive` flag.
  Their SQL expression is *not* surfaced on hover — `Chip` carries a `title` only on an inert
  option, where it gives the reason that option is unreachable.
- **The builder is on all three states** — populated, empty, and the unreadable-permalink error.
  The error state has no query to mutate, so its builder is seeded from the same known-valid
  query the escape link beside it offers; the empty state is the view that most needs its query
  adjusted, so it is the last one that may ship without controls.
- **Choosing a filter VALUE is a route, not a control** — `/explore/filter/:dim` runs a
  one-dimension pivot over the same window and the other active filters, because a chip row
  cannot reach the warehouse. The builder's `f` row is what links to it.
- **A filter chip shows the resolved code and the URL keeps the id.** `f=op_airline_id:19790`
  reads `Carrier = DL`; the filter itself stays on the BTS id, because `dim_carrier` carries the
  *current* carrier code and a code-valued filter would change meaning across a rebuild. The
  page resolves its own filter values — a pivot resolves only the ids in the rows it returned,
  so a dimension filtered but not grouped resolves nothing.
...
**Not built, and the scope of that is the Explorer.** This table has no CSV or Parquet export, on
the result header or anywhere else, and no per-row `rows →` drill into the filed rows behind a
result row — its dimension cells link to the entity pages instead, and narrowing a result is the
builder plus a filter. Do not read that as a product-wide negative: `/watch`'s presets each carry a
real per-row `Raw rows` permalink (`watch/[preset]/page.tsx`), and all four entity pages carry the
page-level form of it in their foot. The export is the one claim that IS product-wide — nothing in
the app emits CSV or Parquet, and the permalink is the only export this product has.
...
>
> **The disclosure is gated on BOTH operands.** Keyed on the grouping alone it fires on every
> mainline view, which has no carrier filter to be inconsistent with; keyed on the filter alone
> it fires on every carrier-filtered *operating* view, where no rollup is happening. A
> single-operand gate has shipped in this repo before (CLAUDE.md, `cardSixthStat`).
...
| **Invalid permalink** | A full-page error naming the offending key and the allowed values, with a link to a valid neighbouring query. Never a silent fallback to defaults — a permalink that quietly renders a different query than it encodes is worse than one that errors, because the screenshot still looks authoritative. **The way out is ONE query, from one definition** (`app/src/lib/pivot/recovery.ts`), and every dead end in the product offers it: both unreadable-permalink states, `/search`'s no-match state, and all six 404s (`/route`, `/carrier`, `/airport`, `/aircraft`, `/watch`, and `/explore/filter/:dim`). **Its window is derived from `dataAsOf()`, never written into source** — `recoveryHref(asOf)`, over the trailing 12 months ending at the dataset's newest month. A frozen window here does not fail loudly: it stays inside the dataset, so it keeps decoding and no gate reddens, and it simply stops being the trailing 12 the product calls it. That is what shipped — `t=2025-05:2026-04` against a `2026-05` warehouse, a month stale on nine surfaces with every gate green. Each call site passes the `asOf` it has already read for its own `DATA AS OF` badge, so this costs no extra query anywhere. A dead end is shown to a reader who is already stuck, so a per-page copy of this link that drifts sends them from one error to another — and a copy that drifts to a *different valid* query is worse, because nothing looks wrong. `/aircraft` is the one deliberate variant, grouped by aircraft type rather than carrier because a reader who reached an aircraft dead end is looking for a type; it is derived from the same query, so it can differ in that one key and no other. |

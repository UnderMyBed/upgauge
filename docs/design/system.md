...
| **Invalid permalink** | A full-page error naming the offending key and the allowed values, with a link to a valid neighbouring query. Never a silent fallback to defaults — a permalink that quietly renders a different query than it encodes is worse than one that errors, because the screenshot still looks authoritative. **The way out is ONE query, from one constant** (`app/src/lib/pivot/recovery.ts`), and every dead end in the product offers it: both unreadable-permalink states, `/search`'s no-match state, and all five 404s. A dead end is shown to a reader who is already stuck, so a per-page copy of this link that drifts sends them from one error to another — and a copy that drifts to a *different valid* query is worse, because nothing looks wrong. `/aircraft` is the one deliberate variant, grouped by aircraft type rather than carrier because a reader who reached an aircraft dead end is looking for a type; it is derived from the same constant, so it can differ in that one key and no other. |

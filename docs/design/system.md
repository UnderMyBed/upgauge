...
**Not built.** There is no CSV or Parquet export, on the result header or anywhere else, and no
per-row `rows →` drill into the filed rows behind a result row. The permalink is the only export
this product has, and a result row's own drill is the builder plus a filter.

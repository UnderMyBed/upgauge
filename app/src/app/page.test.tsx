 ...
 *
 * `bounds.test.ts` used to scan `app/src/app` for hand-spelled `/explore?` literals, pin how many
 * remained, and assert the server still admitted each one. #140 took that count from nine to one;
 * this change takes it to zero, because the last literal was this page's SAMPLE -- and it carried
 * the same frozen `t=2025-05:2026-04` the recovery query did, under the one sentence a first-time
 * visitor actually reads: "the top 25 carriers by seats over the trailing 12 months".
 *
 * A scan asserting emptiness over an empty set is deletable-green, so that scan is gone rather
 * than pinned at zero, and both halves of what it guaranteed are re-stated HERE, beside the query
 * they describe: the exact bytes, and that those bytes decode. `FIXTURE`, not `loadAllowlist()`,
 * for the reason `bounds.test.ts` used it -- the codec half must not need a warehouse.
 */
